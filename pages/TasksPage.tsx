...
import { CheckCircle, Clock, AlertCircle, Filter, Search, ArrowRight, User, Bot, PlusCircle, Briefcase, FileCode, Play, Lock, Send, Terminal, Cpu, RotateCcw, Check, XCircle, Maximize2, Minimize2, MessageSquare, ExternalLink } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-lg ${managerMode === 'AI' ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-300'}`}>
                        {managerMode === 'AI' ? <Bot size={24} /> : <Briefcase size={24} />}
                    </div>
                    <div>
                        <h3 className="font-bold text-white">Current Manager</h3>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="bg-slate-950 p-1 rounded-lg flex border border-slate-700">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${managerMode === 'HUMAN' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-white'}`}
                        >
                            <User size={14} /> Human
                        </button>
                        <button 
                            onClick={() => setManagerMode('AI')}
                            className={`px-4 py-2 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${managerMode === 'AI' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'}`}
                        >
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400">Track progress and deliverables.</p>
                </div>
                <div className="flex gap-2 items-center flex-wrap justify-end">
                    <Button 
                        onClick={handleRequestTask} 
                        disabled={isRequesting}
                        className={`gap-2 ${managerMode === 'AI' ? 'bg-indigo-600 hover:bg-indigo-500' : 'bg-slate-700 hover:bg-slate-600'}`}
                    >
                        {isRequesting ? (
                    ...
                    <div className="bg-slate-800 p-1 rounded-lg border border-slate-700">
                         <div className="relative">
                            ...
                            <input 
                                type="text" 
                                placeholder="Search..." 
                                className="pl-8 pr-4 py-1.5 bg-transparent text-sm text-white focus:outline-none w-32 md:w-48"
                                value={searchTerm}
                    ...
                    <tbody className="divide-y divide-slate-700">
                        {filteredTasks.length === 0 ? (
                            ...
                            filteredTasks.map(task => (
                                <tr key={task.id} onClick={() => handleTaskClick(task)} className="hover:bg-slate-700/30 transition-colors cursor-pointer group">
                                    <td className="p-4">
                                        <div className="font-medium text-white group-hover:text-indigo-400 transition-colors">{task.title}</div>
                                        ...
                                        <td className="p-4">
                                            {task.assigneeId ? (
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${task.assigneeId.includes('AI') ? 'bg-indigo-600 text-white' : 'bg-slate-600 text-white'}`}>
                                    ...
                                    <td className="p-4 text-right">
                                        <ArrowRight size={16} className="ml-auto text-slate-500 group-hover:text-white" />
                                    </td>
                                </tr>
                            ))
                        )}
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-4">
                            <div>
                                ...
                                <h4 className="font-bold text-white text-lg mb-2 leading-tight">{selectedTask.title}</h4>
                                <div className="p-3 bg-slate-900 rounded border border-slate-700 text-sm text-slate-300 leading-relaxed mb-4">
                                ...
                                {selectedTask.aiFeedback && (
                                    <div className="p-3 bg-indigo-900/20 rounded border border-indigo-500/50 mb-4 animate-in fade-in">
                                        <div className="flex items-center gap-2 mb-1">
                                            <Bot size={14} className="text-indigo-400" />
                                            <span className="text-xs font-bold text-indigo-400">AI Manager Feedback</span>
                                        </div>
                                        <p className="text-xs text-slate-300 italic">"{selectedTask.aiFeedback}"</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-2 bg-slate-900 p-4 rounded-lg">
                                <div className="flex justify-between text-xs text-slate-400 border-b border-slate-800 pb-1">
                                    <span>Assignee</span>
                                    <span className="text-white">{selectedTask.assigneeId === currentUser?.id ? 'You' : selectedTask.assigneeId || 'Unassigned'}</span>
                                </div>
                                <div className="flex justify-between text-xs text-slate-400 border-b border-slate-800 pb-1">
                                    <span>Manager</span>
                                    <span className="text-white">{selectedTask.managerType === 'AI' ? 'Atlas (AI)' : 'Sarah (Human)'}</span>
                                </div>
                                <div className="flex justify-between text-xs text-slate-400">
                                    <span>Deadline</span>
                                    <span className="text-white">Sprint End (2d)</span>
                                </div>
                            </div>

                            <div className="mt-4 pt-4 space-y-3 border-t border-slate-700">
                                {!selectedTask.assigneeId ? (
                                    <Button className="w-full bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-900/20" onClick={handleClaimTask}>
                                    ...
                                    selectedTask.status === TaskStatus.BACKLOG ? (
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <Button className="w-full bg-slate-700 hover:bg-slate-600" onClick={handleOpenEditor}>
                                    ...
                                    <div className="bg-slate-800 p-3 rounded text-center text-xs text-slate-500 flex items-center justify-center gap-2">
