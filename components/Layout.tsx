...
const SidebarItem: React.FC<{ icon: any; label: string; to: string; active: boolean; onClick: () => void }> = ({ icon: Icon, label, to, active, onClick }) => (
  <div 
    onClick={onClick}
    className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors cursor-pointer ${active ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
  >
    <Icon size={20} />
...
const NotificationDropdown: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="absolute right-0 top-12 w-80 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-50 max-h-96 overflow-y-auto">
            <div className="p-4 border-b border-slate-700 flex justify-between items-center">
                <h4 className="font-semibold text-white">Notifications</h4>
                <span className="text-xs text-slate-400">{notifications.filter(n => !n.read).length} new</span>
            </div>
            <div className="p-2">
                {notifications.length === 0 ? (
                    ...
                    notifications.map(n => (
                        <div 
                            ...
                            className={`p-3 mb-1 rounded-lg cursor-pointer transition-colors ${n.read ? 'opacity-50 hover:bg-slate-700' : 'bg-slate-700/50 hover:bg-slate-700 border-l-2 border-indigo-500'}`}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className={`text-xs font-bold uppercase ${n.type === 'error' ? 'text-rose-400' : n.type === 'success' ? 'text-emerald-400' : 'text-blue-400'}`}>
                                    {n.type}
                                </span>
  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-800 h-screen fixed left-0 top-0 flex flex-col z-20">
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center gap-3 px-4 py-3 mb-2 cursor-pointer hover:bg-slate-800 rounded-lg transition-colors" onClick={() => navigate('/profile')}>
        ...
        <button onClick={logout} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
          <LogOut size={16} />
          Sign Out
        </button>
      </div>
      ...
      <main className="ml-64 p-8">
        <header className="flex justify-between items-center mb-8 relative">
            <div>
               ...
               <p className="text-slate-400 text-sm">Welcome back to {company.name}</p>
            </div>
            <div className="flex gap-4 items-center">
                ...
                <div className="relative">
                    <button 
                        ...
                        className="p-2 bg-slate-800 rounded-full text-slate-300 hover:bg-slate-700 hover:text-white relative transition-colors"
                    >
                        <Bell size={20} />
                        {unreadCount > 0 && (
